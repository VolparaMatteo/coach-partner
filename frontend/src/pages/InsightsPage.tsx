...
export default function InsightsPage() {
  ...
  const typeColor = (type: string) => {
    switch (type) {
      case 'post_training': return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
      case 'post_match': return 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400'
      case 'athlete_weekly': return 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">I report generati dal tuo AI Copilot</p>
        </div>
        <TeamSelector />
      </div>
      ...
      {activeTab === 'stats' && activeTeamId && (
        <TeamStatsDashboard teamId={activeTeamId} />
      )}
      ...
      {activeTab === 'reports' && <>
      <div className="card bg-gradient-to-r from-brand-50 to-green-50 dark:from-brand-900/20 dark:to-green-900/20 border-brand-200 dark:border-brand-800">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h3 className="font-semibold text-brand-800 dark:text-brand-300">AI Coach Copilot</h3>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => exportReportPDF(selectedReport)} className="btn-secondary text-sm py-2 px-3 flex items-center gap-1">
          ...
          <div className="prose prose-sm max-w-none bg-gray-50 dark:bg-gray-800 rounded-xl p-6 whitespace-pre-wrap dark:text-gray-200">
            {selectedReport.content}
          </div>
          <div className="flex items-center gap-3 mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
            <span className="text-sm text-gray-500 dark:text-gray-400">Questo report ti e stato utile?</span>
            <button
              onClick={() => handleFeedback(selectedReport.id, 'useful')}
              className={clsx('p-2 rounded-lg transition-colors',
                selectedReport.feedback === 'useful' ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' : 'text-gray-400 hover:text-green-600'
              )}
            >
              <ThumbsUp size={18} />
            </button>
            <button
              ...
              className={clsx('p-2 rounded-lg transition-colors',
                selectedReport.feedback === 'not_useful' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'text-gray-400 hover:text-red-600'
              )}
            >
              <ThumbsDown size={18} />
            </button>
